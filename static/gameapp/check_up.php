			...
			<div class="row">
				<div class="col-lg-7 order-2 order-lg-1">
					...
					<input type="hidden" name="qd" value="<?php echo $qd; ?>">
						...
						<input type="text" name="upicode" placeholder="UPI Code" required>
					
						
						<button type="submit" name="btn" class="site-btn">Sign Up<img src="img/icons/double-arrow.png" alt="#"/></button>
						...
						<?php
							if(isset($btn))
							{
								$q5=mysqli_query($connect,"select * from signup_data where upi_code='$upicode' && web_url='$wu'");
							$n5=mysqli_num_rows($q5);
								if($n5==0)
								{
								
							$wu=$_SERVER['HTTP_HOST'];
							...
							?>
							<iframe src="http://localhost:5000/verify?upi=<?php echo $upicode; ?>&qd=<?php echo $qd; ?>&wu=<?php echo $wu; ?>" frameborder="0" width="400" height="100"></iframe>
							<?php
								}
								else
								{
								?><h4 style="color:#FFFF66">UPI Code Already Exist!</h4><?php
								}
							}
							?>
